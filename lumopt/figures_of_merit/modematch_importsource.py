...
class ModeMatchDoubleSource(ModeMatch):
    ...
    def add_adjoint_sources(self, sim):
        ...
        ls.add_imported_source(sim.solver_handle,adjoint_injection_mode,fommonitorname=self.monitor_name,wavelengths=self.wavelengths,direction=adjoint_injection_direction)
        ls.add_imported_source(sim.solver_handle,adjoint_injection_mode,sourcename='opposite_source',fommonitorname=self.monitor_name,wavelengths=self.wavelengths,direction=self.direction)

        return


class ModeMatchSingleSourcenoE(ModeMatch):
