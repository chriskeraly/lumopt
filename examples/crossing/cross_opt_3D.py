...
crossing_base = os.path.join(os.path.dirname(__file__),'crossing_base_TE_modematch_3D.lsf')

######## DEFINE SPECTRAL RANGE #########
wavelengths = Wavelengths(start = 1300e-9, stop = 1600e-9, points = 21)

######## DEFINE OPTIMIZABLE GEOMETRY ########
def cross(params = np.linspace(0.25e-6, 0.6e-6, 10)):
    y_end = params[-1]
    x_end = 0 - y_end
...
polygon_geometry = FunctionDefinedPolygon(func = cross, initial_params = np.linspace(0.25e-6, 0.6e-6, 10), bounds = [(0.2e-6, 1e-6)]*10, z = 0.0, depth = 220.0e-9, eps_out = Material(1.44 ** 2), eps_in = Material(2.8 ** 2, mesh_order = 2), edge_precision = 5, dx = 1.0e-9)

######## DEFINE FIGURE OF MERIT ########
mode_fom = ModeMatch(monitor_name = 'fom', mode_number = 1, direction = 'Forward', multi_freq_src = False, target_T_fwd = lambda wl: np.ones(wl.size), norm_p = 1)

######## DEFINE OPTIMIZATION ALGORITHM ########
scipy_optimizer = ScipyOptimizers(max_iter=10, method = 'L-BFGS-B', scaling_factor = 1e6, pgtol = 1e-3)
